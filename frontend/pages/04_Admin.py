...
admin_section = st.sidebar.radio(
    "Select Section",
    ["Dashboard Overview", "User Management", "API Keys", "System Configuration", "Audit Logs", "Backups"],
)
...
def get_mock_users() -> List[Dict]:
    return [
        {"id": 1, "name": "Admin User", "email": "admin@refurbadmin.co.za", "role": "admin", "status": "active"},
        {"id": 2, "name": "John Smith", "email": "john@example.co.za", "role": "user", "status": "active"},
        {"id": 3, "name": "Jane Doe", "email": "jane@example.co.za", "role": "user", "status": "inactive"},
    ]

@st.cache_data
def get_mock_api_keys() -> List[Dict]:
    return [
        {"id": 1, "name": "Production API", "key": "sk_prod_***", "created": "2024-01-01", "last_used": "2024-01-15", "status": "active"},
        {"id": 2, "name": "Development API", "key": "sk_dev_***", "created": "2024-01-05", "last_used": "2024-01-14", "status": "active"},
    ]

@st.cache_data
def get_mock_audit_logs() -> List[Dict]:
    return [
        {"id": 1, "event": "user.login", "user": "admin", "timestamp": "2024-01-15 10:30:00", "ip": "192.168.1.***"},
        {"id": 2, "event": "data.export", "user": "john", "timestamp": "2024-01-15 09:15:00", "ip": "192.168.1.***"},
        {"id": 3, "event": "config.change", "user": "admin", "timestamp": "2024-01-14 16:45:00", "ip": "192.168.1.***"},
    ]
...
# =============================================================================
    ...
    with action_col3:
        if st.button("⚠️ Deactivate User", use_container_width=True):
...
# =============================================================================
    ...
    with st.expander("➕ Create New API Key"):
        ...
        key_expiry = st.date_input("Expiry Date", value=datetime.now() + timedelta(days=365))
...
# =============================================================================
    ...
    with col1:
        event_type = st.selectbox("Event Type", ["All", "auth", "data", "admin", "security"])
    ...
    selected_log = st.selectbox("Select Log Entry", [f"#{l['id']}: {l['event']}" for l in logs])
...
# =============================================================================
    ...
    backup_history = [
        {"filename": "backup_20240115_020000.sql.gz", "size": "24.5 MB", "created": "2024-01-15 02:00:00", "status": "✓"},
        {"filename": "backup_20240114_020000.sql.gz", "size": "24.3 MB", "created": "2024-01-14 02:00:00", "status": "✓"},
        {"filename": "backup_20240113_020000.sql.gz", "size": "24.1 MB", "created": "2024-01-13 02:00:00", "status": "✓"},
    ]
...
# Footer
st.markdown("---")
st.caption(f"RefurbAdmin AI v1.0.0 | Last updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | 🇿🇦 South Africa")
